      ...
      types: [
        nexus.extendType({
          ...
          definition(t) {
            ...
            // retorno la imagen como objeto
    
            t.field("avatar_image", {
              type: "UploadFileEntity",
              resolve: async (parent, args, ctx) => {
                const userImage = await strapi.query('plugin::users-permissions.user').findOne({
                  where: {
                    id: parent.id,
                  },
                  populate: {
                ...
                return userImage?.avatar_image;
              }
            });
            t.field("role", {
              type: "UsersPermissionsRole",
              resolve: async (parent, args, ctx) => {
                const userWithRole = await strapi.query("plugin::users-permissions.user")
                  .findOne({
                    where: {
                      id: parent.id,
                    },
            ...
            t.field("meta", {
              ...
              resolve: async (parent, args, ctx) => {
                ...
                const MetaUser = await strapi.query("api::meta-user.meta-user")
                  .findOne({
                    where: {
                      user: parent.id,
                    }
